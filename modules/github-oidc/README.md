...
- A policy da role `infra` é inline (máximo 10.240 caracteres por role): ~9.800 em 2026-09-26. Inclui `logs:*LogDelivery`, as ações de tag do Budgets (access log da API HTTP da plataforma) e `iam:CreateServiceLinkedRole` para `ops.apigateway.amazonaws.com` e `email.cognito-idp.amazonaws.com` (e-mail das user pools pelo SES). O teste `infra_policy_allows_cognito_email_service_linked_role` falha antes de estourar o teto.
